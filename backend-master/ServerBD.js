...
app.post('/usuario/inserir', async (req, res) => {
  console.log('Requisição recebida para criar novo usuário:', req.body); // Log para verificar os dados recebidos
  ...
  try {
    ...
    const novoUsuario = await Usuario.create({
      nome: nome.trim(),
      email: email.trim(),
      senha: senha.trim(), // Você pode adicionar lógica de hash de senha aqui
      tipoUsuario,
    });
    ...
    res.status(500).json({ error: 'Erro ao tentar inserir o usuário.' });
  }
});
